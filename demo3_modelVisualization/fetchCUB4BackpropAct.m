function [imo, imBackup ]= fetchCUB4BackpropAct(images, mode, varargin)
...
for i=1:numel(images)
    ...
    for ai = 1:opts.numAugments
        ...
        if ~isempty(opts.averageImage)
            ...
            flipudFlag = rand(1);
            if strcmp(mode, 'train') && flipudFlag >0.5
                A = bsxfun(@minus, imt(sy,sx,:), offset) ;
                imo(:,:,:,si) = flipud(A);
            elseif strcmp(mode, 'train') && flipudFlag <= 0.5 

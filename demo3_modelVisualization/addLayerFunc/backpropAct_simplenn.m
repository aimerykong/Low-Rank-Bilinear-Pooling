function res = backpropAct_simplenn(net, x, dzdy, res, varargin)
...
%             res(i+1).x = my_nnloss(res(i).x, l.class) ;
            res(i+1).x = actEucLoss_nnloss(res(i).x, l.class) ;
...
%         disp(l);
        switch l.type           
            case 'conv'
                if length(size(res(i+1).dzdx)) == 2
                    res(i+1).dzdx = reshape(res(i+1).dzdx, [1, 1, size(res(i+1).dzdx)]);
                end
            ...
            case 'loss'
                res(i).dzdx = actEucLoss_nnloss(res(i).x, l.class, res(i+1).dzdx) ;

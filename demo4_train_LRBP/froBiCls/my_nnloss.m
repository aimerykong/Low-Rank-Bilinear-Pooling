function Y = my_nnloss(X,c,dzdy,varargin)
...
opts.classWeights = [] ;
opts.threshold = 0 ;
opts.loss = 'softmaxlog' ; % softmaxlog hinge
opts.topK = 5 ;
...
%             t = max(0, 1 - c.*X) ;            
            ...
            XX = squeeze(X);
            cc = single(squeeze(c));
            CC = single(-1*ones(size(XX)));
            for i =1:size(XX,2)
                CC(cc(i),i) = 1;
            end
            t = max(0, 1 - CC.*XX) ;
            t(t>thresh) = t(t>thresh)*slopFactor;
        case 'hinge_backup'
%             t = max(0, 1 - c.*X) ;            
            % one-vs-all hinge loss
            XX = squeeze(X);
...
%             t(t>thresh) = t(t>thresh)*slopFactor;
            ...
            Y = (CC.*XX < 1);
            Y(t>thresh) = Y(t>thresh)*slopFactor;
            Y = - dzdy .* CC .* Y;
        case 'hinge_backup' % hinge_backup
            %Y = - dzdy .* c .* (c.*X < 1) ; % t = max(0, 1 - c.*X) ;            
            % one-vs-all hinge loss
            XX = squeeze(X);
            cc = single(squeeze(c));
            CC = single(-1*ones(size(XX)));
...
%{
t_mhinge = max(0, 1 - X(ci));
t_hinge = max(0, 1 - c.*X);

[~,predLabel] = max(squeeze(X),[],1);

[predLabel(:), squeeze(c), predLabel(:)~=squeeze(c), squeeze(t_mhinge), squeeze(t_hinge)]

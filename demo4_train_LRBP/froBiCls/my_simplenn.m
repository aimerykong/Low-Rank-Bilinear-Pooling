...
%     end
    ...
    switch l.type
        ...
        case 'loss'
            res(i+1).x = my_nnloss(res(i).x, l.class) ;
        ...
        case 'relu'
            ...
            %           disp(' ');
            %       end
            if isfield(l, 'leak') && l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
    ...
    if i > 1
        ...
        forget = forget && (~needsBProp || (strcmp(l.type, 'relu') && ( ~isfield(lp, 'precious') || (isfield(lp, 'precious') && ~lp.precious)) ));
        forget = forget && ~(strcmp(lp.type, 'loss') || strcmp(lp.type, 'softmaxloss')) ;
        forget = forget && ( ~isfield(lp, 'precious') || (isfield(lp, 'precious') && ~lp.precious));
    end
    if forget
        res(i).x = [] ;
    end
...
if doder
    ...
    for i=n:-1:backPropLim
        ...
        switch l.type
            ...
            case 'conv'
                %disp(i);
                if size(res(i+1).dzdx, 1) ~= 1 && size(size(res(i+1).dzdx),2) ~= 1 && i >= length(net.layers)-1
                    [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
                        vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, reshape(res(i+1).dzdx, [1,1,size(res(i+1).dzdx)]), ...
                        'pad', l.pad, ...
                        'stride', l.stride, ...
                        l.opts{:}, ...
                        cudnn{:}) ;                    
            ...
            case 'loss'
                res(i).dzdx = my_nnloss(res(i).x, l.class, res(i+1).dzdx) ;
...
%         fprintf('%s\n', l.name);
%         fprintf('\tmean(abs(dzdx)): %.11f\n', mean(abs(res(i).dzdx(:))) );
%         switch l.type
%             case 'conv'
%                 fprintf('\tmean(abs(dzdw)): %.4f\n', mean(abs(dzdw{1}(:))) );        
%         end

...
weight_file = fullfile('imdbFolder', dataset, 'exp', ['classifierW_' dataset '_' network '_' initMethod '_' ...
    classificationType '_' cfgId '_' num2str(inputImgSize) '_' postfix '.mat']);
...
elseif strcmp(initMethod, 'FroBiSVM') % using the Frobenius Bilinear SVM to pretrain the classifier
    ...
    elseif usePCA_onWeights % applying PCA on the weights from full linear SVM
        ...
        trainFV = reshape(trainFV, [512^2, numel(trainFV)/512^2]);
        valFV = reshape(valFV, [512^2, numel(valFV)/512^2]);
        ...
        for i = 1:size(W,3)
            ...
            diagSr  = diag(Sr);
            a = find(diagSr>=0);
            if length(a) > rDimBiCls
                U(:,:,i) = Ur(:,a(1:rDimBiCls)) * diag( diagSr(a(1:rDimBiCls)).^0.5 ) ;
            else
                acomp = find(diagSr<0);
                aconcat = [a, acomp(end:-1:end-1-(rDimBiCls-length(a))) ];
                U(:,:,i) = Ur(:, aconcat) * diag( diagSr(aconcat).^0.5 ) ;
            end
        end
        ...
        % save on disk
        savefast(weight_file, 'initFCparam', 'U', 'b', 'acc', 'map', 'scores', 'w', 'rDimBiCls');

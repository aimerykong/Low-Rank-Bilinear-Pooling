...
%{
for i = 25:numel(net.layers)
    if isfield(net.layers{i}, 'weights')
        J = numel(net.layers{i}.weights) ;
        if J~=0 && strcmp(net.layers{i}.type,'conv')
            net.layers{i}.learningRate = 0.0001*ones(1, J, 'single') ;
        elseif J~=0 && ~strcmp(net.layers{i}.type,'conv')
            net.layers{i}.learningRate = 10*[0.1 0.2]; %0*ones(1, J, 'single') ;6
        end
        
        if J~=0 && strcmp(net.layers{i}.type,'conv')
            net.layers{i}.weightDecay = [0.00001,0]; %ones(1, J, 'single') ;
        elseif J~=0 && ~strcmp(net.layers{i}.type,'conv')
            net.layers{i}.weightDecay = [0.00001,0]; %ones(1, J, 'single') ;
        end
    end
...
for i = 18:numel(net.layers)
    if isfield(net.layers{i}, 'weights')
        J = numel(net.layers{i}.weights) ;
        if J~=0 && strcmp(net.layers{i}.type,'conv')
            net.layers{i}.learningRate = 1*0.01*ones(1, J, 'single') ;
        elseif J~=0 && ~strcmp(net.layers{i}.type,'conv')
            net.layers{i}.learningRate = 1*10*[0.1 0.2]; %0*ones(1, J, 'single') ;6
        end
        
        if J~=0 && strcmp(net.layers{i}.type,'conv')
            net.layers{i}.weightDecay = [0.005,0]; %ones(1, J, 'single') ;
        elseif J~=0 && ~strcmp(net.layers{i}.type,'conv')
            net.layers{i}.weightDecay = [0.005,0]; %ones(1, J, 'single') ;
        end
    end
...
net.layers{34}.learningRate = 0.1*[1,2];
net.layers{34}.weightDecay = [0.005,0]; %ones(1, J, 'single') ;

% lambda = 0.0001;
% net.layers{32}.forward = @biClsUUreg_forward;
% net.layers{32}.backward = @biClsUUreg_backward;
% net.layers{32}.weightDecay = [0.0001,0];
% net.layers{32}.lambda = lambda;
%}
